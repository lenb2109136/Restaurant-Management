...
function ThemDon() {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            const data = await layDuLieuTheoID("dscho", id);
            setdui(data)
            setdon(recreateDon(data))
            data.update=true
            capNhatDuLieuTheoID("dscho", id, data)
      ...
      useEffect(()=>{
        return async ()=>{
            const data = await layDuLieuTheoID("dscho", params.get("id"));
            let i= parseInt(params.get("id"))
            i=i+1

            const data1 = await layDuLieuTheoID("dscho", i);
            if(data!=null||data!=undefined){
                data.update=false
                capNhatDuLieuTheoID("dscho", params.get("id"), data)
            }
            if(data1!=null||data1!=undefined){
                data1.update=false
                capNhatDuLieuTheoID("dscho", i, data1)
            }
            ...
            <div style={{ display: "flex", color: "white" }} >
                ...
                {don?.length != 0 ? <button style={{ marginLeft: "40px", backgroundColor: "#7AB730", color: "white", borderRadius: "10px", padding: "15px", marginBottom: "30px" }} onClick={() => {
                    ...
                    don.forEach(data => {
                        ...
                        console.log("dối tượng gửi đi ")
                        send.id=parseInt(id)
                        console.log(send)
                    axios.post('http://localhost:8080/yeucau/capnhat', send, {
                        headers: {
                        ...
                        .then(response => {
                            ...
                            xoaDuLieuTheoID("dscho",id)
                            alert("Cập nhật đơn thành công")
                            navigate(`/phucvu/capnhatdangcho?id=${h.id}`)
                            setdon([])
                        })
                        .catch(error => {
