...
import Add from "../public/addmonan"
import ChiTiet from "../public/chitietmonan"
...
export default function (prop) {
    ...
    useEffect(() => {
        ...
        axios.get("/getloaimonan")
            .then(d => {
                ...
                setloai(d.data.data[0].tenLoai);
            })
            .catch(() => alert("Không thể lấy dữ liệu"));

        axios.get(`/monan/byloai?idloai=1`)
            .then(d => {
                let km = [...d.data.data];
                let requests = km.map(item =>
                    axios.get(`http://localhost:8080/monan/getkhuyenmai?id=${item.id}`)
                        .then(response => ({
                            ...item,
    ...
    return (
        <div style={{ border: "2px solid rgb(67, 94, 33)",borderRadius:"20px", position: "fixed", top: "20%", right: "3px", backgroundColor: "white",width:"95%",marginRight:"2%", }}>
            <div><img onClick={() => {
                prop.setopen(false)
            ...
            <div className="d-flex justify-content-center mt-2">
                ...
                {openi ? <ChiTiet data={dt} se={seti} setds={setdsmonan} maloai={maloai}></ChiTiet> : null}
                        ...
                        return (
                            <p onClick={() => {
                                ...
                                axios.get(`/monan/byloai?idloai=${data.id}`)
                                    ...
                                    .catch((e) => alert(e));
                            }} className="ms-2" style={{ borderRight: "1px solid black", paddingRight: "10px", cursor: "pointer", color: loai === data.tenLoai ? "#7AB730" : "black" }}>
                                {data.tenLoai}
                            </p>
                        )
                    })
            ...
            <hr id="hr" className="ms-3 w-100" style={{ backgroundColor: "#7AB730" }}></hr>
            {
                nott ? (<div className=" ms-4 row d-flex align-item-center justify-content-center">
                    <img style={{ maxHeight: "300px", maxWidth: "300px" }} className="h-100 w-100" src="https://cdn-icons-gif.flaticon.com/17569/17569424.gif"></img>
                </div>) : null
            }

            <div className="mt-3 ms-4 row menu " style={{ backgroundColor: '#7AB730',paddingBottom:"30px", cursor: "pointer", borderRadius: "10px", maxHeight: "600px", overflowY: "scroll" }}>
                        ...
                        return (
                            <div style={{ borderBottom: "1px solid white", marginLeft: "2px" }} className="row mt-2" key={data.id} >
                                ...
                                <div className="mb-3 ms-2 text-white col-lg-2 col-md-2 col-12 mt-2 text-center d-sm-flex justify-content-center align-items-center mx-auto">
                                    ...
                                    }} defaultChecked={findIndexById(prop.ds[prop.index].dsma, data.id) != -1 ? true : false} type="checkbox"></input>
                                </div>
                                {
                    ...
                    <div style={{
                                ...
                                km[id.current].khuyenmai.kmtt.map((data, index) => {
                                    return <div>
                                        <span>{data.KM_TEN}:  <Button onClick={() => {
                                            axios.get(`http://localhost:8080/monan/ngungkmtt?idma=${data.MA_ID}&idkm=${data.KM_ID}`)
                                                .then((data) => {
                        ...
                        {km[id.current].khuyenmai.kmgv.length != 0 ? <div>
                            <strong><p style={{ textAlign: "center" }}>Khuyến mãi giờ vàng</p></strong>
                            {
                                ...
                                km[id.current].khuyenmai.kmgv.map(data => {
                                    return <div>
                                        <span>{data.KM_TEN}-</span>
                                        <Button>Ngưng áp dụng</Button>
                                    </div>
                                })
                            }
                        </div> : null}
